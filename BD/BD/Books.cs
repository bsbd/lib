...
namespace BD5
{
    public partial class Books : Form
    {
        ...
        DataTable GetComments()
        {
            ...
            {
                Program.conn.Open();
                NpgsqlCommand command = new NpgsqlCommand("SELECT * FROM books ORDER BY id_books LIMIT " + y + " offset " + j, Program.conn);
                NpgsqlCommand command1 = new NpgsqlCommand("SELECT COUNT(*) FROM books", Program.conn);
        ...
        private void button2_Click(object sender, EventArgs e)
        {
            ...
            {
                Program.conn.Open();
                string sqlQuery = "INSERT INTO books(publication_date,publisher,price) VALUES (@add1, @add2, @add3)";
        ...
        private void button3_Click(object sender, EventArgs e)
        {
            ...
            {
                Program.conn.Open();
                string sqlQuery = "DELETE FROM books WHERE id_books = @select";
                NpgsqlCommand command = new NpgsqlCommand(sqlQuery, Program.conn);
                try
                {
        ...
        private void button4_Click(object sender, EventArgs e)
        {
            ...
            {
                Program.conn.Open();
                if (comboBox3.Text == "publication_date")
                {
                    string sqlQuery = "UPDATE books SET publication_date = @select1 WHERE id_books = @select2";
                    NpgsqlCommand command = new NpgsqlCommand(sqlQuery, Program.conn);
                    try
                    {
                ...
                {
                    string sqlQuery = "UPDATE books SET publisher = @select1 WHERE id_books = @select2";
                    NpgsqlCommand command = new NpgsqlCommand(sqlQuery, Program.conn);
                    try
                    {
                ...
                {
                    string sqlQuery = "UPDATE books SET price = @select1 WHERE id_books = @select2";
                    NpgsqlCommand command = new NpgsqlCommand(sqlQuery, Program.conn);
                    try
                    {
